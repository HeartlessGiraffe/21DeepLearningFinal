...
sys.argv = ['']
del sys
parser = argparse.ArgumentParser(description='PyTorch CIFAR100 Training Mixup')
...
# mixup主体函数
def mixup_data(x, y, alpha=args.alpha, use_cuda=True):
...
# mixup损失函数，原图像label和替换图像label的交叉熵损失函数的线性组合
...
if __name__ == '__main__':
    ...
    print(net)
    # 创建tensorboard文件夹
    writer_train_loss = SummaryWriter('./runs' + args.dataset + args.model + '/train/' + 'mixup')
    writer_test_acc = SummaryWriter('./runs' + args.dataset + args.model + '/test/' + 'mixup')
    writer_test_acc5 = SummaryWriter('./runs' + args.dataset + args.model + '/test_top5/' + 'mixup')
    ...
    with open(logname, 'a') as logfile:
        ...
        logwriter.writerow(
            ['dataset:', args.dataset, 'model:', args.model, 'arg_method:', 'mixup', 'epoch:', args.epoch,
             'batch_size',
